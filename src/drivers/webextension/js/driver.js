...
const Driver = {
  ...
  async init() {
    ...
    if (!previous) {
      // First install: clear caches. Geen welcome-tab — Lekkertje is
      // self-contained en opent niets bij install/upgrade.
   ...
   */
  async getDetections(hostnameArg) {
    let url
    let hostname

    if (hostnameArg) {
      // Lekkertje: expliciete hostname (test-harness / popup met ?domain=).
      // Skip active-tab lookup en setIcon.
    ...
    const resolved = (cache ? resolve(cache.detections) : []).filter(
      ({ lastUrl }) => showCached || !url || isSimilarUrl(url, lastUrl)
    )
  ...
  /**
   * Lekkertje is self-contained: NO telemetry, ever.
   * Wappalyzer's stock ping() POST'te detected technologies en ads-tracking
   * naar externe endpoints. Beide weg. Reset alleen counters zodat Driver-
   * callsites niet hoeven te wijzigen.
   */
  ping() {
